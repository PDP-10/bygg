...
parsevalue pval;		/* Holds return values etc. */
...
static fdb* this;		/* The FDB we are trying now. */
...
static void help_keyword(keytab* kt)
{
  keyword* kw;			/* current keyword. */
...
static void try_keyword(void)
{
  ...
  if (recogflag) {
	...
	cm_fillstr(&(((char*) pval.kw->value)[atomptr]),
		   strlen((char*) pval.kw->value) - atomptr);
	if (!cm_tbluk(kt, (char*) pval.kw->value) || !pval.tbl.exact) {
...
static int try_ip6addr(void)
{
  static unsigned char fyfan[] = {
    0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0x00, 0x1f,	/*  !"# $%&' ()*+ ,-./ 0123 4567 89:; <=>? */
...
static bool makedyn(keytab* kt, bool grow)
{
  keyword* newtab;
  int newsize;
  int i;
  ...
  newtab = malloc(sizeof(keyword) * newsize);
...
bool cm_tbadd(keytab* kt, char* name, int offset)
{
  int i;
  keyword* kw;
...
bool cm_tbdel(keytab* kt, char* name)
{
  int i;
...
bool cm_tbluk(keytab* kt, char* name)
{
  int i;
  int namelen;
  keyword* kw;
...
keytab* cm_ktab(keyword* keys, int flags)
{
  dyn_keytab* dk;
  keytab* k;
  int i;
...
fdb* cm_fdb(int type, char* help, int flags, void* data)
{
  dyn_fdb* df;
  fdb* f;
...
static void simpleparse(int function, char* help, int flags, char* data)
{
  fdb f = { 0 };
...
{
  (*(void (*)(void)) pval.kw->value)();	/* URRRK. */
}

void cm_pkey(char* help, int flags, keyword* keys, int kflags)
{
  simpleparse(_CMKEY, help, flags, (char*) cm_ktab(keys, kflags));
}

void cm_pcmd(char* help, int flags, keyword* keys, int kflags)
{
