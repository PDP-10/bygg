...
#    define CM_WLD 0x0002	/*   Allow wildcards. */
  struct fdb * next;		/* Link to next fdb in chain. */
...
/* We need a type to hold different types of data... */
...
typedef struct {
  fdb* used;			/* The FDB that parsed successfully. */
  keyword* kw;			/* Pointer to keyword struct that matched. */
...
extern void  cm_pkey(char* help, int flags, keyword* keys, int kflags);
extern void  cm_pcmd(char* help, int flags, keyword* keys, int kflags);
...
extern void  cm_parse(fdb* chain);

extern fdb*  cm_chain(fdb* first, ...);
extern fdb*  cm_fdb(int type, char* help, int flags, void* data);
extern keytab* cm_ktab(keyword* kw, int flags);

extern bool  cm_tbadd(keytab* kt, char* name, int offset);
extern bool  cm_tbdel(keytab* kt, char* name);
extern bool  cm_tbluk(keytab* kt, char* name);
