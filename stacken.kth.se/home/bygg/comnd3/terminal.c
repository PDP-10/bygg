...
static void iohandler(int sig)
{
  int i;
  
  while ((subjob != NULL) && ((i = fgetc(subjob)) >= 0)) {
    if (i == '\n') {
      term_input('\r');
    }
    term_input(i);
...
static void term_opensub(void)
{
  int ivalue;

  if (subjob == NULL) {
    (void) signal(SIGPIPE, pipehandler);
    /* (void) signal(SIGIO, iohandler); */
    (void) signal(SIGALRM, iohandler);
    subjob = popen("script /dev/null", "r+");
    if (subjob != NULL) {
      ivalue = 1;
      if (ioctl(fileno(subjob), FIONBIO, &ivalue) < 0) {
	perror("FIONBIO");
      }
      /*
      ivalue = getpid();
      if (ioctl(fileno(subjob), FIOSETOWN, &ivalue) < 0) {
	perror("FIOSETOWN");
      }
      ivalue = 1;
      if (ioctl(fileno(subjob), FIOASYNC, &ivalue) < 0) {
	perror("FIOASYNC");
      }
      */
      alarm(1);
