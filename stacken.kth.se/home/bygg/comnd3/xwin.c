...
static winindex openup(int wintype)
{
  windowblock* win;
  int width, height;
...
winindex w_open(int wintype)
{
  if (wintype == wty_terminal) {
    term_init();
  }
  return (openup(wintype));
}
...
void w_setinput(winindex index, void (*input)(char))
{
  windowblock* win;
