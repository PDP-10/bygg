...
static dispblock maindisp[256] = {
  ...
  { 0x69, 1, notyet, 0,        0 },	/* imul */
  { 0x6a, 0, inst,   0,        "push %1" },
  { 0x6b, 1, notyet, 0,        0 },	/* imul */
  { 0x6c, 2, inst,   0,        "insb" },
  { 0x6d, 2, inst,   0,        "insw" },
  { 0x6e, 0, inst,   0,        "outsb" },
  { 0x6f, 0, inst,   0,        "outsw" },
  ...
  { 0x9e, 0, inst,   0,        "sahf" },
  { 0x9f, 0, inst,   0,        "lahf" },
  { 0xa0, 0, inst,   0,        "mov $al,[%2]" },
  { 0xa1, 0, inst,   0,        "mov $ax,[%2]" },
  { 0xa2, 0, inst,   0,        "mov [%2],$al" },
  { 0xa3, 0, inst,   0,        "mov [%2],$ax" },
  { 0xa4, 0, inst,   0,        "movsb" },
  { 0xa5, 0, inst,   0,        "movsw" },
  { 0xa6, 0, inst,   0,        "cmpsb" },
  { 0xa7, 0, inst,   0,        "cmpsw" },
...
static dispblock pfx0Fdisp[] = {
  ...
  { 0xc8, 4, inst,   0,        "bswap reg" },
  { 0xc9, 4, inst,   0,        "bswap reg" },
  { 0xca, 4, inst,   0,        "bswap reg" },
  { 0xcb, 4, inst,   0,        "bswap reg" },
  { 0xcc, 4, inst,   0,        "bswap reg" },
  { 0xcd, 4, inst,   0,        "bswap reg" },
  { 0xce, 4, inst,   0,        "bswap reg" },
  { 0xcf, 4, inst,   0,        "bswap reg" },
  { 0x00, 0, arnold, 0,        0 },
};
...
static void emitea(void)
{
  if (ea32a) {
	...
	emitsize();
      ...
      emitpfx(easeg);
      bufchar('['); number(getlong()); bufchar(']');
    } else {
      switch (eamode) {
	...
	emitsize();
      ...
      emitpfx(easeg);
      bufchar('['); number(getword()); bufchar(']');
    } else {
      switch (eamode) {
...
static void emitdisp(void)
{
  ...
  if (eawb) {
    if (ea32a) {
      offset = sextl(getlong());
    } else {
      offset = sextw(getword());
    }
  } else {
    offset = sextb(getbyte());
  }
...
static bool computelength(char* expand)
{
  ...
  while ((c = *(expand++)) != (char) 0) {
	...
	break;
      case ':':
	pb_length += 4;		/* Hrlzm, even in p-mode? */
	break;
      case 'd':
	dcl();
	break;
...
static void doinstr(void)
{
  ...
  case instb:
    ...
    break;
  case instw:
    suggest(touch, st_word, 2);	/* Hm, 32-bit? */
    break;
  case pushj:
    pb_detour = touch;
    break;
...
void i8086_init(void)
{
  ...
  setregs();			/* Set up register indexes. */
}
