...
extern bool updateflag;		/* If we are updating things. */
...
static bool unixflag;		/* True if unix syntax. */
...
{
  switch (assembler) {
  case assembler_foreign:
  case assembler_unix:
    unixflag = true;
    break;
  case assembler_native:
  default:
    unixflag = false;
    break;
  }
...
void m68k_sset(symindex index)	/* Symbol got set. */
{
  checksyntax();		/* Re-setup the unix flag. */
}
...
static void setpf(void)
{
  ...
  checksyntax();		/* Set up the unix flag. */
}
