...
int parse_number(char* defaultanswer, char* helpstring)
 {
  ...
  parse(fdbchn(&hexfdb, &countfdb, nil), &parseval, &used);
  if (used == &hexfdb) {
...
/*
** parse_value() parses a value, which for the moment is exactly the
** same thing as a number.
*/

longword parse_value(char* defaultanswer, char* helpstring)
{
  return(parse_number(defaultanswer, helpstring));
}
...
address* parse_address(char* dfault)
{
  address* addr;
...
static address* pa_first = nil;
static address* pa_last = nil;
int pa_count;

address* parse_range(char* dfault, char* buzzword, fdb* nextfield)
{
  static fdb hyphfdb = { _CMTOK, TOK_WAK, nil, (pdat) "-", nil, nil, nil };
  static fdb colonfdb = { _CMTOK, TOK_WAK, nil, (pdat) ":", nil, nil, nil };
  
  pa_first = a_copy(parse_address(dfault), pa_first);

  hyphfdb._cmlst = &colonfdb;
  colonfdb._cmlst = nextfield;

  parse(&hyphfdb, &parseval, &used);
  if (used == &hyphfdb) {
    pa_last = a_copy(parse_address(nil), pa_last);
    pa_count = 2;
    if (buzzword) {
      noise(buzzword);
    }
    parse(nextfield, &parseval, &used);
  } else if (used == &colonfdb) {
    pa_last = a_copy(pa_first, pa_last);
    (void) parse_number(nil, nil);
    if (pa_last != nil) {
      a_inc(pa_last, (parseval._pvint - 1));
    }
    pa_count = 2;
    if (buzzword) {
      noise(buzzword);
...
");
    ...
    parse(fdbchn(&fmtfdb, &cfmfdb, &redfdb, &pipfdb, nil), &parseval, &used);
    if (used == &fmtfdb) {
      format = parseval._pvint;
      parse(fdbchn(&cfmfdb, &redfdb, &pipfdb, nil), &parseval, &used);
    }
...
");
    ...
    pos = a_copy(parse_address(a_a2str(getsfirst(1))), pos);
...
");
	...
	bufaddress(subrange);
	bufstring(" value=");
	bufnumber(r_read(index, subrange));
...
void mem_copy(void)
{
  if (help()) {
    bufstring("\
\n\
Syntax:  MEMORY COPY (from) <address> (to) <address> (count) <number>\n\
\n\
This command copies a block of memory from one address to another.\n\
\n\
");
  } else {
    static address* fromaddr = nil;
    static address* toaddr = nil;
    longword count;
    
    noise("from");
    fromaddr = a_copy(parse_address(nil), fromaddr);

    noise("to");
    toaddr = a_copy(parse_address(nil), toaddr);

    noise("count");
    count = getsrest(fromaddr);
    if (count != 0) {
      count = parse_number(l2s(count), nil);
    } else {
      count = parse_number(nil, nil);
    }
...
void mem_exclude(void)
{
  if (help()) {
    bufstring("\
\n\
Syntax:  MEMORY EXCLUDE <arguments>\n\
\n\
...
void mem_include(void)
{
  if (help()) {
    bufstring("\
\n\
Syntax:  MEMORY INCLUDE (from) <address> (count) <number>\n\
\n\
...
void mem_move(void)
{
  if (help()) {
    bufstring("\
\n\
Syntax:  MEMORY MOVE (from) <address> (to) <address> (count) <number>\n\
\n\
...
");
    ...
    addr = a_copy(parse_address(nil), addr);
    value = parse_text("data to store");
...
static address* peekaddress = nil;
...
");
    ...
    noise("at");
    pos = a_copy(parse_address(a_a2str(defpeek)), pos);
    ...
    peekaddress = a_copy(pos, peekaddress);
    bufclose();
  }
}
...
");
    ...
    noise("value");
    value = parse_value(nil, "value of register");

    addr = parse_regrange();

    index = r_define(regname);
    r_write(index, addr, value);
  }
}
...
");
    ...
    static keywrd cmd[] = {
      { "default",  0,  (keyval) casing_default },
      { "lower",    0,  (keyval) casing_lower },
      { "initial",  0,  (keyval) casing_initial },
      { "upper",    0,  (keyval) casing_upper },
    };
...
#define  opdelim_default   0	/*   Default delimiter. */
#define  opdelim_space     1    /*   Single space before args. */
#define  opdelim_tab       2    /*   Tab before args. */
...
");
    ...
    static keywrd cmd[] = {
      { "default",  0,  (keyval) opdelim_default },
      { "space",    0,  (keyval) opdelim_space },
      { "tab",      0,  (keyval) opdelim_tab },
    };
    static keytab cmdtab = { (sizeof(cmd)/sizeof(keywrd)), cmd };
    ...
    switch (parseval._pvint) {
      case opdelim_default: value = nil;  break;
      case opdelim_space:   value = " ";  break;
      case opdelim_tab:     value = "\t"; break;
    }
    confirm();

    s_write(s_define("opdelim"), value);
  }
}
...
");
    ...
    static keywrd cmd[] = {
      { "default",  0,  (keyval) radix_default },
      { "octal",    0,  (keyval) radix_octal },
      { "decimal",  0,  (keyval) radix_decimal },
      { "hex",      0,  (keyval) radix_hex },
    };
    static keytab cmdtab = { (sizeof(cmd)/sizeof(keywrd)), cmd };
...
");
    ...
    static keywrd cmd[] = {
      { "default",  0,  (keyval) assembler_default },
      { "native",   0,  (keyval) assembler_native },
      { "foreign",  0,  (keyval) assembler_foreign },
      { "unix",     0,  (keyval) assembler_unix },
    };
    static keytab cmdtab = { (sizeof(cmd)/sizeof(keywrd)), cmd };
...
");
    ...
    noise("at address");
    addr = parse_range(nil, "to", fdb_pattern());

    pat = parse_pattern(true, nil);
    paecheck();

    if (pa_count == 1) {
      while (pat != nil) {
	size = pe_size(addr, pat);
	setstatus(addr, pat->status, size);
	a_inc(addr, size);
	pat = pat->next;
      }
    } else {
      count = a_diff(pa_last, pa_first) + 1;
      pstart = pat;
      while (count > 0) {
	size = pe_size(addr, pat);
	setstatus(addr, pat->status, size);
...
");
    ...
    noise("of address");
    addr = a_copy(parse_address(a_a2str(getsfirst(1))), addr);
    noise("count");
    count = getsrest(addr);
    if (count > (64 * 20)) {
      count = 64*20;
...
void cmd_window(void)
{
  ...
  static fdb numfdb = { _CMNUM, NUM_US, nil, (pdat) 10, nil, nil, nil };
  static fdb openfdb = { _CMTOK, 0, nil, (pdat) "open", nil, nil, nil };
  static fdb cfmfdb = { _CMCFM, 0, nil, nil, nil, nil, nil };

  argwinindex = 0;		/* Clear target index. */

  if (helpflag) {
    parse(fdbchn(&winfdb, &openfdb, &cfmfdb, nil), &parseval, &used);
  } else {
    parse(fdbchn(&winfdb, &numfdb, &openfdb, nil), &parseval, &used);
    if (used == &numfdb) {
  ...
  if (used == &cfmfdb) {
    bufstring("\
\n\
The WINDOW command is used to handle variuos subwindows.\n\
\n\
");
  } else if (used == &openfdb) {
    win_open();
  } else {
    execute(parseval._pvfunc);
  }
...
void win_move(void)
{
  if (help()) {
    bufstring("\
\n\
Syntax:  WINDOW MOVE <direction> <amount>\n\
\n\
...
");
    ...
    confirm();
    w_move(argwinindex, direction, amount);
  }
}
...
void win_next(void)
{
  ...
  if (!dispatch(&nxtfdb)) {
    bufstring("\
\n\
The [WINDOW] NEXT command is used to move the window in various ways.\n\
\n\
");
  }
}
...
void wnxt_description(void)
{
  if (help()) {
    bufstring("\
\n\
Syntax:  [WINDOW] NEXT DESCRIPTION\n\
\n\
This command moves the window forward to the next address with a\n\
description.\n\
\n\
...
void wnxt_reference(void)
{
  if (help()) {
    bufstring("\
\n\
Syntax:  [WINDOW] NEXT REFERENCE <addr>\n\
\n\
...
void wnxt_status(void)
{
  if (help()) {
    bufstring("\
\n\
Syntax:  [WINDOW] NEXT STATUS <status code>\n\
\n\
...
\n\
This command opens a new window of the specified type..\n\
\n\
");
  } else {
    static keywrd cmds[] = {
