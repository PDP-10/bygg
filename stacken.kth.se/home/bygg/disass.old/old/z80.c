...
#define CH	0x04		/* One-byte arg might be a char. */
#define H64	0x08		/* 64180 only instr. */
#define I85     0x10		/* 8085 instruction. */
...
extern bool delayblank;		/* If we want a blank line after the object. */

extern int assembler;		/* Native, ... */
...
static void copytext(char* p)
{
  ...
  while ((c = *(p++)) != (char) 0) {
    if (c == '%') {
      c = *(p++);
      if ((c == '1') && (argstatus == st_char)) {
	c = 'c';
      }
      if ((c == '2') && (argstatus == st_ptr)) {
	c = 'a';
      }
      switch (c) {
	...
	refaddr(getword());
	break;
      case 'c':		/* One byte, possibly character */
	b = getbyte();
	if ( printable(b) &&
	    (updateflag) &&
	    (!c_exist(istart)) ) {
...
static void checksyntax(void)
{
  ...
  if (assembler == assembler_foreign) {
    if (!zilogflag) {
      forcez = true;
    }
  }
...
{
  checksyntax();		/* Check for assembler. */
}
