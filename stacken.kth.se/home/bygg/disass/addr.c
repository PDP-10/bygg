...
/*
** a_touch() is an UGLY HACK for the time beeing.
*/

address* a_touch(address* a)
{
  static address* work = NULL;
...
/* XXX fix the following two routines when actually doing 64-bits. */

/*
** a_q2a() converts a quadword (64 bits) to an address.
*/

address* a_q2a(quadword q)
{
  return a_l2a(q);
}

/*
** a_a2q() converts an address (if possible) to a (64 bit) quadword.
*/

quadword a_a2q(address* a)
{
  return a_a2l(a);
}
...
address* a_split(address* addr, address* split)
{
  ...
  case aty_range:		/* range, addr-last. */
    a_range(split, a_copy(a_last(addr), NULL));
    a_set(addr->next, a_prev(work));
    return split;
  case aty_count:		/* count, addr:n */
    a_range(split, a_copy(a_last(addr), NULL));
    a_range(addr, a_copy(a_prev(work), NULL));
    return split;
  default:
...
** the tail of the first argument.  If the first argument is NULL,
** nothing happens.
*/
...
address* a_cdr(address* a)
{
  ...
  if ((a->type == aty_range) && (a->next != NULL)) {
    return a->next->next;
  }
