...
static FILE* workfile = NULL;
static char* openname = NULL;
...
{
  savename(filename, 'b');
  if ((workfile = fopen(filename, "rb")) == NULL) {
    return false;
  }
  return true;
}
...
/*
** wf_wbopen() opens the i/o channel for binary writing.
*/

bool wf_wbopen(char* filename)
{
  savename(filename, 'b');	/* THIS IS WRONG */
  if ((workfile = fopen(filename, "wb")) == NULL) {
    return false;
  }
  return true;
}
...
/*
** wf_setpos() sets the next position for reads.  Inteded for binay files.
*/

void wf_setpos(int pos)
{
  if (workfile != NULL) {
    (void) fseek(workfile, pos, SEEK_SET);
  }
}
...
/*
** wf_wvalue() writes a value to the i/o stream.
*/

void wf_wvalue(value* v)
{
  switch (v->type) {
    case vty_long: wf_whex(v->idata); break;
    case vty_addr: wf_waddr(v->pdata); break;
  }
}
...
/*
** wf_wblock() writes a block of binary data.
*/

void wf_wblock(byte* buffer, int size)
{
  (void) fwrite(buffer, 1, size, workfile);
}
