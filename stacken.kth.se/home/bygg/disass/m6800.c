...
static void m6800_doinstr(void)
{
  byte opcode;
  dispblock* disp;
  ...
  switch (disp->itype) {
    case pfx18:
      disp = finddisp(getbyte(), pfx18disp, NULL);
      break;
    case pfx1A:
      disp = finddisp(getbyte(), pfx1Adisp, NULL);
      break;
    case pfxCD:
      disp = finddisp(getbyte(), pfxCDdisp, NULL);
      break;
    case unused:
      disp = NULL;
      break;
...
static void setpf(void)
{
  ...
  spf_dodef(m6800_dobyte);

  spf_doobj(st_inst, m6800_doinstr);
  spf_doobj(st_byte, m6800_dobyte);
  spf_doobj(st_word, m6800_doword);
  spf_doobj(st_ptr,  m6800_doptr);
  spf_doobj(st_char, m6800_dochar);
  spf_doobj(st_text, m6800_dotext);

  spf_begin(m6800_begin);
  spf_end(m6800_end);
  spf_org(m6800_org);
