...
static void copyindex(byte postbyte)
{
  ...
  if (postbyte & 0x80) {
    number((postbyte & 0x1f));	/* DO SIGN EXTEND! */
    bufchar(',');
    casechar(ixreg);
  } else {
    if (postbyte & 0x10) {
    ...
    case 8:  number(sextb(getbyte())); bufchar(','); casechar(ixreg); break;
    case 9:  number(sextb(getword())); bufchar(','); casechar(ixreg); break;
    case 10: /* does not exist */
    case 11: casechar('D'); bufchar(','); casechar(ixreg); break;
    case 12: number(sextb(getbyte())); bufchar(','); casestring("PCR"); break;
    case 13: number(sextb(getword())); bufchar(','); casestring("PCR"); break;
    case 14: /* does not exist */
    case 15: number(getword()); break;
    }
    if (postbyte & 0x10) {
      bufchar(']');
    }
...
static void m6809_doinstr(void)
{
  byte opcode;
  dispblock* disp;
  ...
  switch (disp->itype) {
  case pfx10:
    disp = finddisp(getbyte(), pfx10disp, NULL);
    break;
  case pfx11:
    disp = finddisp(getbyte(), pfx11disp, NULL);
    break;
  case unused:
    disp = NULL;
    break;
...
void m6809_init(void)
{
  ...
  spf_dodef(m6809_dobyte);

  spf_doobj(st_inst, m6809_doinstr);
  spf_doobj(st_byte, m6809_dobyte);
  spf_doobj(st_word, m6809_doword);
  spf_doobj(st_ptr,  m6809_doptr);
  spf_doobj(st_char, m6809_dochar);
  spf_doobj(st_text, m6809_dotext);

  spf_begin(m6809_begin);
  spf_end(m6809_end);
  spf_org(m6809_org);
