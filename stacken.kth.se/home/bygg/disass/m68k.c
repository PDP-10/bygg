...
static address* moveatarget = NULL; /* Address that got loaded. */
static address* moveanext = NULL;   /* Address of next instruction. */
...
static void brarg(long disp)
{
  ...
  target = a_touch(a_offset(istart, disp));
  reference(target);
  if (l_exist(target)) {
...
static char* cregname(int reg)
{
  ...
  static struct foobar {
    ...
    { 0x000,   "sfc" },		/* 10/20/30/40/CPU32 */
    { 0x001,   "dfc" },		/* 10/20/30/40/CPU32 */
    { 0x002,  "cacr" },		/*    20/30/40 */
    { 0x003,    "tc" }, 	/*          40/LC040 */
    { 0x004,  "itt0" }, 	/*          40/LC040 */
    { 0x004, "iacr0" }, 	/*             EC040 */
    ...
    { 0x007,  "dtt1" }, 	/*          40/LC040 */
    { 0x007, "dacr1" }, 	/*             EC040 */
    { 0x800,   "usp" },		/* 10/20/30/40/CPU32 */
    { 0x801,   "vbr" },		/* 10/20/30/40/CPU32 */
    { 0x802,  "caar" },		/*    20/30 */
    { 0x803,   "msp" },		/*    20/30/40 */
    { 0x804,  "ispa" },		/*    20/30/40 */
    { 0x805, "mmusr" }, 	/*          40/LC040 */
    { 0x806,   "urp" }, 	/*          40/LC040 */
    { 0x807,   "srp" }, 	/*          40/LC040 */
  };
#endif
...
static void m68k_dochar(void)
{
  defc(getbyte());
}

static void m68k_doword(void)
{
  word w;
...
static void setpf(void)
{
  ...
  spf_docomment(m68k_docomment);
  spf_dodescr(m68k_dodescr);

  spf_dodef(m68k_dobyte);

  spf_doobj(st_inst, m68k_doinstr);
  spf_doobj(st_byte, m68k_dobyte);
  spf_doobj(st_word, m68k_doword);
  spf_doobj(st_long, m68k_dolong);
  spf_doobj(st_ptr,  m68k_doptr);
  spf_doobj(st_char, m68k_dochar);
  spf_doobj(st_text, m68k_dotext);

  spf_begin(m68k_begin);
  spf_end(m68k_end);
  spf_org(m68k_org);
