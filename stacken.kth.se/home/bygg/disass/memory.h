...
extern bool mapnostat(address* addr);
...
extern void f_insert(address* addr, pattern* pat);
extern pattern* f_read(address* addr);
extern void f_write(address* addr, int pos, int field, int arg);
#  define FLD_STATUS 1
#  define FLD_RADIX  2
#  define FLD_SIGN   3
...
extern void m_set(address* addr, byte* buf, int len);
