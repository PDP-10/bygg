...
static void pdp10_startline(bool nonempty)
{
  if (listformat) {
    if (nonempty) {
      bufchar(statuschar);
...
static void pdp10_restline(void)
{
  /* not needed yet */
}
...
static void pdp10_doinstr(void)
{
  byte ac;
  address* a;
...
void pdp10_init(void)
{
  ...
  spf_startline(pdp10_startline);
  spf_restline(pdp10_restline);

  spf_dodef(pdp10_doword);

  spf_doobj(st_inst, pdp10_doinstr);
  spf_doobj(st_word, pdp10_doword);
  spf_doobj(st_ptr,  pdp10_doptr);
  spf_doobj(st_text, pdp10_dotext);
  
  spf_begin(pdp10_begin);
  spf_end(pdp10_end);
  spf_org(pdp10_org);
