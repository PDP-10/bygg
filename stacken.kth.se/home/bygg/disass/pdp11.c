...
/*
** The routines below should try to align the output data to word
** boundaries (even addresses).  That is, objects that are longer
** than two bytes should try to align to the next word boundary as
** soon as they can, i.e. by outputting a single byte and then doing
** words.  Needs some thinking.
*/
...
static void pdp11_startline(bool nonempty)
{
  byte lsb, msb;
...
static void pdp11_restline(void)
{
  if (residue > 0) {
...
static void genlabel(address* addr)
{
  ...
  if (!l_exist(addr)) {
    w = a_a2w(addr);
    if (w > 077777) {
      sprintf(work, "U%05" PRIow, w & 077777);
    } else {
      sprintf(work, "L%05" PRIow, w);
    }
    l_insert(addr, work);
  }
}
...
/*
** pcomma() moves from one arg to the next.
*/
...
static void pea77(void)
{
  bufchar('@');
  pea67();
  /* we can be called by a two-operand instr, for the first op.
  ** think this over.
  if (!c_exist(istart)) {
    tabto(32);
    bufchar(';');
...
static void peaddr(int mode, int reg)
{
  ...
  switch (mode) {
    case 7: bufchar('@');
    case 6: pea6x(reg); return;
      /* number(getword()); pireg(reg); return; */
    case 5: bufchar('@');
...
static void pdp11_doinstr(void)
{
  int i;

  if (a_a2w(istart) & 1) {	/* Odd address? */
    defb(getbyte());
    return;
  }
...
void pdp11_init(void)
{
  ...
  spf_startline(pdp11_startline);
  spf_restline(pdp11_restline);

  spf_dodef(pdp11_dobyte);

  spf_doobj(st_inst, pdp11_doinstr);
  spf_doobj(st_byte, pdp11_dobyte);
  spf_doobj(st_word, pdp11_doword);
  spf_doobj(st_ptr,  pdp11_doptr);
  spf_doobj(st_char, pdp11_dochar);
  spf_doobj(st_text, pdp11_dotext);

  spf_begin(pdp11_begin);
  spf_end(pdp11_end);
  spf_org(pdp11_org);
