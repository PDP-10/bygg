...
static dispblock maindisp[256] = {
  ...
  { 0x8f, 1, popj,   CL,  "caseb %b,%b,%b" }, /* XXX */
  ...
  { 0xaf, 1, popj,   CL,  "casew %w,%w,%w" }, /* XXX */
  ...
  { 0xcf, 1, popj,   CL,  "casel %l,%l,%l" }, /* XXX */
...
static void putreg(byte reg)
{
  switch (reg) {
    ...
    default:  casechar('r'); bufdecimal(reg, 1); break;
  }
}
...
static void disp(long offset, byte reg, bool indirect)
{
  ...
  if (reg == 0x0f) {		/* PC? */
    touch = a_touch(a_offset(pc, offset));
    if (l_exist(touch)) {
      reference(touch);
...
static void refarg(char type)
{
  ...
  indexreg = 0;			/* Needed to make gcc shut up. */
...
static void vax_doinstr(void)
{
  ...
  switch (disp->itype) {
  case pfxFD:
    disp = finddisp(getbyte(), pfxFDdisp, NULL);
    break;
  case unused:
    disp = NULL;
    break;
...
void vax_init(void)
{
  ...
  spf_dodef(vax_dobyte);

  spf_doobj(st_inst, vax_doinstr);
  spf_doobj(st_byte, vax_dobyte);
  spf_doobj(st_word, vax_doword);
  spf_doobj(st_long, vax_dolong);
  spf_doobj(st_ptr,  vax_doptr);
  spf_doobj(st_char, vax_dochar);
  spf_doobj(st_text, vax_dotext);
  spf_doobj(st_mask, vax_domask);
