...
evf_init rabbit_init;
evf_help rabbit_help;

struct entryvector rabbit_vector = {
  "rabbit",			/* Name */
  "Rabbit RCM3000",		/* One-liner. */
  rabbit_init,			/* Init routine */
  rabbit_help,			/* Help routine */
};
...
#define pfxRB  12		/* Rabbit (i/o) prefixes. */

/* flags: */

#define H64	0x01		/* 64180 only instr. */
#define I85     0x02		/* 8085 instruction. */
#define RB	0x04		/* Rabbit instruction. */
...
static dispblock rabbitdisp[256] = {
  { 0x00, 1, inst,   0,   "nop" },
  { 0x01, 3, inst,   0,   "ld bc,%2" },
  { 0x02, 1, inst,   0,   "ld (bc),a" },
  { 0x03, 1, inst,   0,   "inc bc" },
  ...
  { 0x25, 1, inst,   0,   "dec h" },
  { 0x26, 2, inst,   0,   "ld h,%1" },
  { 0x27, 2, inst,   RB,  "add sp,%7" }, /* Should possibly be sign-ext. */
  { 0x28, 2, pushj,  0,   "jr z,%d" },
  { 0x29, 2, inst,   0,   "add hl,hl" },
  { 0x2A, 3, instw,  0,   "ld hl,(%a)" },
  { 0x2B, 1, inst,   0,   "dec hl" },
  { 0x2C, 1, inst,   0,   "inc l" },
  { 0x2D, 1, inst,   0,   "dec l" },
  ...
  { 0xCA, 3, pushj,  0,   "jp z,%a" },
  { 0xCB, 2, pfxCB,  0,   0 },
  { 0xCC, 1, inst,   RB,  "bool hl" },
  { 0xCD, 3, pushj,  0,   "call %a" },
  { 0xCE, 2, inst,   0,   "adc a,%1" },
  { 0xCF, 4, inst,   RB,  "lcall %:" },
  { 0xD0, 1, inst,   0,   "ret nc" },
  { 0xD1, 1, inst,   0,   "pop de" },
  { 0xD2, 3, pushj,  0,   "jp nc,%a" },
  { 0xD3, 1, inst,   RB,  "ioi" },
  { 0xD4, 2, inst,   RB,  "ld (%s),%x" },
  { 0xD5, 1, inst,   0,   "push de" },
  { 0xD6, 2, inst,   0,   "sub %1" },
  { 0xD7, 1, trap,   0,   "rst 10h" },
  { 0xD8, 1, inst,   0,   "ret c" },
  ...
  { 0xDA, 3, pushj,  0,   "jp c,%a" },
  { 0xDB, 1, inst,   RB,  "ioe" },
  { 0xDC, 1, inst,   RB,  "and hl,de" },
  { 0xDD, 0, pfxDD,  0,   0 },
  { 0xDE, 2, inst,   0,   "sbc a,%1" },
  { 0xDF, 1, trap,   0,   "rst 18h" },
  { 0xE0, 1, inst,   RB,  "ret lz" },
  { 0xE1, 1, inst,   0,   "pop hl" },
  { 0xE2, 3, pushj,  RB,  "jp lz,%a" },
  { 0xE3, 1, inst,   RB,  "ex de',hl" },
  { 0xE4, 2, inst,   RB,  "ld hl,(%y)" },
  { 0xE5, 1, inst,   0,   "push hl" },
  { 0xE6, 2, inst,   0,   "and %1" },
  { 0xE7, 1, trap,   0,   "rst 20h" },
  { 0xE8, 1, inst,   RB,  "ret lo" },
  { 0xE9, 1, popj,   0,   "jp (hl)" },
  { 0xEA, 3, pushj,  RB,  "jp lo,%a" },
  { 0xEB, 1, inst,   0,   "ex de,hl" },
  { 0xEC, 1, inst,   RB,  "or hl,de" },
  { 0xED, 0, pfxED,  0,   0 },
  { 0xEE, 2, inst,   0,   "xor %1" },
  { 0xEF, 1, trap,   0,   "rst 28h" },
  { 0xF0, 1, inst,   0,   "ret p" },
  { 0xF1, 1, inst,   0,   "pop af" },
  { 0xF2, 3, pushj,  0,   "jp p,%a" },
  { 0xF3, 1, inst,   RB,  "rl de" },
  { 0xF4, 2, inst,   RB,  "ld (%y),hl" },
  { 0xF5, 1, inst,   0,   "push af" },
  { 0xF6, 2, inst,   0,   "or %1" },
  { 0xF7, 1, inst,   RB,  "mul" },
  { 0xF8, 1, inst,   0,   "ret m" },
  { 0xF9, 1, inst,   0,   "ld sp,hl" },
  { 0xFA, 3, pushj,  0,   "jp m,%a" },
  { 0xFB, 1, inst,   RB,  "rr de" },
  { 0xFC, 1, inst,   RB,  "rr hl" },
  { 0xFD, 0, pfxFD,  0,   0 },
  { 0xFE, 2, inst,   0,   "cp %1" },
  { 0xFF, 1, trap,   0,   "rst 38h" },
};

/* ---- rabbit DD/FD prefix table ---- */

static dispblock rabbitDDdisp[] = {
  { 0x09, 2, inst,   0,   "add %x,bc" },
  { 0x19, 2, inst,   0,   "add %x,de" },
  { 0x21, 4, inst,   0,   "ld %x,%2" },
  { 0x22, 4, instw,  0,   "ld (%a),%x" },
  ...
  { 0x56, 3, inst,   0,   "ld d,(%y)" },
  { 0x5E, 3, inst,   0,   "ld e,(%y)" },
  { 0x64, 2, inst,   RB,  "ldp (%x),hl" },
  { 0x65, 4, instw,  RB,  "ldp (%2),%x" },
  { 0x66, 3, inst,   0,   "ld h,(%y)" },
  { 0x6C, 2, inst,   RB,  "ldp hl,(%x)" },
  { 0x6D, 4, instw,  RB,  "ldp %x,(%2)" },
  { 0x6E, 3, inst,   0,   "ld l,(%y)" },
  { 0x70, 3, inst,   0,   "ld (%y),b" },
  { 0x71, 3, inst,   0,   "ld (%y),c" },
  { 0x72, 3, inst,   0,   "ld (%y),d" },
  ...
  { 0x75, 3, inst,   0,   "ld (%y),l" },
  { 0x77, 3, inst,   0,   "ld (%y),a" },
  { 0x7C, 2, inst,   RB,  "ld hl,%x" },
  { 0x7D, 2, inst,   RB,  "ld %x,hl" },
  { 0x7E, 3, inst,   0,   "ld a,(%y)" },
  { 0x86, 3, inst,   0,   "add a,(%y)" },
  { 0x8E, 3, inst,   0,   "adc a,(%y)" },
  { 0x96, 3, inst,   0,   "sub (%y)" },
  ...
  { 0xB6, 3, inst,   0,   "or (%y)" },
  { 0xBE, 3, inst,   0,   "cp (%y)" },
  { 0xC4, 3, inst,   RB,  "ld %x,(%s)" },
  { 0xD4, 3, inst,   RB,  "ld (%s),%x" },
  { 0xDC, 2, inst,   RB,  "and %x,de" },
  { 0xCB, 4, pfxCB,  0,   0 },
  { 0xCC, 2, inst,   RB,  "bool %x" },
  { 0xE1, 2, inst,   0,   "pop %x" },
  { 0xE3, 2, inst,   0,   "ex (sp),%x" },
  { 0xE4, 3, inst,   RB,  "ld hl,(%y)" },
  { 0xE5, 2, inst,   0,   "push %x" },
  { 0xE9, 2, popj,   0,   "jp (%x)" },
  { 0xEC, 2, inst,   RB,  "or %x,de" },
  { 0xF4, 3, inst,   RB,  "ld (%y),hl" },
  { 0xF9, 2, inst,   0,   "ld sp,%x" },
  { 0xFC, 2, inst,   RB,  "rr %x" },
  { 0,    0, arnold, 0,   0 },
};
...
static dispblock rabbitEDdisp[] = {
  { 0x41, 2, inst,   RB,  "ld bc',de" },
  { 0x42, 2, inst,   0,   "sbc hl,bc" },
  { 0x43, 4, instw,  0,   "ld (%a),bc" },
  { 0x44, 2, inst,   0,   "neg" },
  { 0x45, 2, popj,   RB,  "lret" },
  { 0x46, 2, inst,   RB,  "ipset 0" },
  { 0x47, 2, inst,   RB,  "ld eir,a" },
  { 0x49, 2, inst,   RB,  "ld bc',bc" },
  { 0x4A, 2, inst,   0,   "add hl,bc" },
  { 0x4B, 4, instw,  0,   "ld bc,(%a)" },
  { 0x4D, 2, popj,   0,   "reti" },
  { 0x4E, 2, inst,   RB,  "ipset 2" },
  { 0x4F, 2, inst,   RB,  "ld iir,a" },
  { 0x51, 2, inst,   RB,  "ld de',de" },
  { 0x52, 2, inst,   0,   "sbc hl,de" },
  { 0x53, 4, instw,  0,   "ld (%a),de" },
  { 0x54, 2, inst,   RB,  "ex (sp),hl" },
  { 0x56, 2, inst,   RB,  "ipset 1" },
  { 0x57, 2, inst,   RB,  "ld a,eir" },
  { 0x59, 2, inst,   RB,  "ld de',bc" },
  { 0x5A, 2, inst,   0,   "adc hl,de" },
  { 0x5B, 4, instw,  0,   "ld de,(%a)" },
  { 0x5D, 2, inst,   RB,  "ipres" },
  { 0x5E, 2, inst,   RB,  "ipset 3" },
  { 0x5F, 2, inst,   RB,  "ld a,iir" },
  { 0x61, 2, inst,   RB,  "ld hl',de" },
  { 0x62, 2, inst,   0,   "sbc hl,hl" },
  { 0x63, 4, instw,  RB,  "ld (%a),hl" },
  { 0x64, 2, inst,   RB,  "ldp (hl),hl" },
  { 0x65, 4, instw,  RB,  "ldp (%2),hl" },
  { 0x67, 2, inst,   RB,  "ld xpc,a" },
  { 0x69, 2, inst,   RB,  "ld hl',bc" },
  { 0x6A, 2, inst,   0,   "adc hl,hl" },
  { 0x6C, 2, inst,   RB,  "ldp hl,(hl)" },
  { 0x6D, 4, instw,  RB,  "ldp hl,(%2)" },
  { 0x72, 2, inst,   0,   "sbc hl,sp" },
  { 0x73, 4, instw,  0,   "ld (%a),sp" },
  { 0x76, 2, inst,   RB,  "push ip" },
  { 0x77, 2, inst,   RB,  "ld a,xpc" },
  { 0x7A, 2, inst,   0,   "adc hl,sp" },
  { 0x7B, 4, instw,  0,   "ld sp,(%a)" },
  { 0x7E, 2, inst,   RB,  "pop ip" },
  { 0xA0, 2, inst,   0,   "ldi" },
  { 0xA8, 2, inst,   0,   "ldd" },
  { 0xB0, 2, inst,   0,   "ldir" },
  { 0xB8, 2, inst,   0,   "lddr" },
...
static int family;		/* Processor family: */
#  define fam_intel  1		/*   Intel 8080/8085. */
#  define fam_zilog  2		/*   Zilog Z80. (also H64180) */
#  define fam_rabbit 3		/*   Rabbit. */

static bool h64flag;		/* True if 64180 */
static bool i85flag;		/* True if 8085 */

static bool forcez;		/* Force Z80 mnemonics for 8080/8085. */
...
{
  /* XXX on a rabbit, can be under the influence of an i/o prefix. */
...
static void decode(dispblock* disp)
{
  ...
  case pfxDD:
    ixreg = IX;
    if (family == fam_rabbit) {
      decode(finddisp(getbyte(), rabbitDDdisp, NULL));
    } else {
      decode(finddisp(getbyte(), DDdisp, NULL));
    }
    return;
  case pfxED:
    if (family == fam_rabbit) {
      decode(finddisp(getbyte(), rabbitEDdisp, NULL));
    } else {
      decode(finddisp(getbyte(), EDdisp, NULL));
    }
    return;
  case pfxFD:
    ixreg = IY;
  ...
  if (forcez && !(disp->flags & I85)) {	/* Forcing Z80 mnemonics? */
...
static void z80_doinstr(void)
{
  ixreg = HL;		/* Default index reg. */
  opcode = getbyte();
  /* clear rabbit flags here */
  switch (family) {
  case fam_intel:
  ...
  case fam_zilog:
    ...
    break;
  case fam_rabbit:
    decode(&rabbitdisp[opcode]);
    break;
  }
}

static void z80_dotext(void)
{
  int pos;
  char c;
  char* line;
...
static void setpf(void)
{
  ...
  spf_dodef(z80_dobyte);

  spf_doobj(st_inst, z80_doinstr);
  spf_doobj(st_byte, z80_dobyte);
  spf_doobj(st_word, z80_doword);
  spf_doobj(st_ptr,  z80_doptr);
  spf_doobj(st_char, z80_dochar);
  spf_doobj(st_text, z80_dotext);
...
bool rabbit_help(int helptype)
{
  if (helptype == hty_general) {
    bufstring("\
Help string for Rabbit RCM3000 processor.\n\
");
    return true;
  }
  return false;
