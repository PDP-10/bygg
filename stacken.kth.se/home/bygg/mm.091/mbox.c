...
/*
 * mbox_divider:
 * try to be more intelligent than just looking for "From " at the
 * beginning of a line.
 */
...
mbox_divider(char* p)
{
  ...
  if (strncmp(p, key, keysize) != 0) { /* OK so far? */
    return (false);
  }
  ...
  while (!isspace (*p) && *p != '\0') {
    if (*p == '"')		/* ignore inter-quote stuff */
      while (*(++p) != '"' && *p != '\0');
    p++;
  }

  while (isspace (*p) && *p != '\0') /* Skip to date/time part. */
    p++;
  ...
  atptr = strchr(fromptr, '@');

  if (atptr == NULL)		/* We must have an atsign in the address. */
    return (false);
  if (atptr >= dateptr)
    return (false);

  /* try to parse the date here? */
...
int num;
{
	...
	} while (!mbox_divider(header)); /* till new msg */
    }					/* do next message */
...
/*    date = stringtotime (cp); */
    ...
    } while (!eof && !mbox_divider(&text[line]));
