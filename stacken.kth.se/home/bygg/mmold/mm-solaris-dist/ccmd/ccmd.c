...
extern int (*(preact[]))();	/* standard action table */
...
csb cmcsb = {			/* CSB for all parsing */
	...
	1000,			/* max help tokens */
	NULL,			/* nonblocking I/O handler */
	-1,			/* wrap column */
	NULL,			/* command history */
	-1,			/* max rows */
	NULL,			/* no current position yet */
	preact,			/* prefix actions */
};
...
**
** As a side effect, the first time this is called, the CCMD environment
** variable is read and interpreted.  This doesn't really belong here, but
** there is no other init routine which must get called first...
**/
...
int cmdlen,atomlen,worklen;
{
  ...
  cmcsb._cmcur = cmcsb._cmptr = cmdbuf;	/* no parsed characters in buffer */
...
char *p;
{
  ...
  cmcsb._cmcur = cmcsb._cmptr = cmcsb._cmbfp;
  cmcsb._cminc = 0;
...
fdb **usedfdb;
{
  ...
  int clean = !(cmcsb._cmflg & CM_DRT); /* if buffer is clean */
  ...
  if (ret == CMxOK)		/* still OK? */
	...
	clean = FALSE;
	if (cmcsb._cmptr + cmcsb._cminc > cmcsb._cmbfp)
	    cmcsb._cmflg |= CM_DRT;
	if (ret == CMxEOF) {
	    if (checkeof(fdblist)) {
  ...
  if ((ret != CMxOK) && (cmcsb._cmerh != NULL)) { /* handlable error? */
    ...
    for(f = fdblist; f ; f = f->_cmlst) /* check if there is a specialized */
      if (f->_cmest) {			/* error string. */
					/* and invoke the handler */
	ret = (*cmcsb._cmerh)(ret, f->_cmest,f->_cmffl);
	break;
      }
    ret = (*cmcsb._cmerh)(ret, NULL, 0); /* and invoke the handler */
  }
...
brktab *btab;
{
  ...
  if (!blocked &&		/* if we're not blocked */
	...
	ret = cmsti1(c,CC_NEC); /* and put back confirm char */
      if (ret == CMxOK)
	*(cmcsb._cmcur) = cc; /* and restore flags */
    }
...
brktab *btab;
{
  ...
  while (TRUE) {		/* at most twice, if input ends with */
				...
				/* loop until no more unparsed data */

    while (cmcsb._cmcur - cmcsb._cmptr > cskip) {
...
cmhst(n) {
	...
	if (n < h->len) {		/* making history smaller */
