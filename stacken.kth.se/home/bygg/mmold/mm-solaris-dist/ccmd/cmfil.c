...
PASSEDSTATIC char *default_ext();
PASSEDSTATIC char *partial();
PASSEDSTATIC do_stat();
PASSEDSTATIC int fil_parse(), fil_help(), fil_complete(), fil_match();
PASSEDSTATIC int fnamlen();
PASSEDSTATIC pvfil buildexactfilelist(),buildpartialfilelist();
PASSEDSTATIC validate();

char *malloc(), *tilde_expand();
					/* standard break mask */
  ...
  *cpltlen = 0;
  ...
  if (pcount == 0) {			/* no matches. */
	...
	if (fblk->def_extension) {
	  ...
	  *cplt = empty;			/* accept unmatching text */
	  *cpltlen = 0;
	  goto done;
	}
      }
...
int fcount;
{
  ...
  for(start = 0; start < strlen(buf); start++) /* skip leading dots */
    if (buf[start] != '.') break;
