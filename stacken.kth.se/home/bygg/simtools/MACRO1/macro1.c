...
enum pseudo_t
{
  DECIMAL, DEFINE, FLEX, CONSTANTS, OCTAL, REPEAT, START
};
...
#define DAC 0240000			/* incorrect */
#define DIO 0320000			/* correct read-in instruction */
#define JMP 0600000
...
BOOL pseudoOperators( PSEUDO_T val )
{
  ...
  case DEFINE:
    ...
    for( length = 0;; )
    {
      ...
      if( strcmp( termin, "TERMIN" ) == 0) break;
      if (index == 0)
      {
        length = copyMacLine( length, 0, strlen( line ), count - 1);
      }
    }
    if( length < 0 )
    {
  ...
  case FLEX:
    pack = 0;
    if( lexstart < maxcc )
      pack = pack | ( ascii_to_fiodec[line[lexstart++]] << 12 );
    if( lexstart < maxcc )
      pack = pack | ( ascii_to_fiodec[line[lexstart++]] << 6 );
    if( lexstart < maxcc )
      pack = pack | ascii_to_fiodec[line[lexstart++]];
