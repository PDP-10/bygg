...
byte txret[8];			/* returned hash. */
byte hash[8];			/* my hash value. */
byte fullkey[256];		/* key, with salt. */
int keylen;

struct arc4_stream txs, rxs, hss;
...
void arc4_hash(void)
{
  ...
  arc4_init(&hss);
  for (i = 0; i < 256; i += 1) {
    (void) arc4_getbyte(&hss);
  }
  for (i = 0; i < 8; i += 1) {
    hash[i] = arc4_getbyte(&hss);
  }
}
...
int do_auth(void)
{
  ...
  if (!send8(txsalt)) goto fail;
  if (!recv8(rxsalt)) goto fail;
  if (memcmp(txsalt, rxsalt, 8) == 0) goto fail;
  setupkey(rxsalt);
  arc4_hash();
  if (!send8(hash))   goto fail;
  if (!recv8(txret))  goto fail;
  (void) alarm(0);

  setupkey(txsalt);
  arc4_hash();
  if (memcmp(txret, hash, 8) != 0) {
    return 0;			/* Not equal, fail. */
  }
  setupkey(txsalt);
  arc4_init(&txs);
  setupkey(rxsalt);
