...
int rp_Attach(UNIT *rpUnit, char *fileName)
{
	...
	if (st = disk_Open(rpUnit, fileName, 0))
		return st;
...
int rp_Format(UNIT *rpUnit)
{
	...
	homBlock[0000] = util_PackedASCII6("HOM");
	homBlock[0001] = util_PackedASCII6("TIM000");
	...
	for (idx1 = 0, idx2 = 0; idx2 < RP_BLKSZ18; idx1++, idx2 += 5) {
		util_Convert36to8(homBlock[idx1], &rp_cBuffer[idx2]);
	}
	disk_Seek(rpUnit, 0, SEEK_SET);
	disk_Write(rpUnit, rp_cBuffer);
...
int rp_WriteBlock(UNIT *rpUnit)
{
	...
	for (idx = 0; idx < RP_BLKSZ18; idx += 5) {
		if (st = rhDevice->CheckWordCount(rhUnit))
			break;
		rhDevice->ReadData36(rhUnit, &data36);
		util_Convert36to8(data36, &rp_cBuffer[idx]);
	}

	if (idx)
		return disk_Write(rpUnit, rp_cBuffer);
	else
		return st;
}
...
int rp_CheckBlock(UNIT *rpUnit)
{
	...
	if ((st = disk_Read(rpUnit, rp_cBuffer)) >= 0) {
		for (idx = 0; idx < RP_BLKSZ18; idx += 5) {
			data1 = util_Convert8to36(&rp_cBuffer[idx]);
			rhDevice->ReadData36(rhUnit, &data2);
			if (data1 != data2)
				return -2;
			if (st = rhDevice->CheckWordCount(rhUnit))
				return st;
...
int rp_ReadBlock(UNIT *rpUnit)
{
	...
	if ((st = disk_Read(rpUnit, rp_cBuffer)) >= 0) {
		for (idx = 0; idx < RP_BLKSZ18; idx += 5) {
			data36 = util_Convert8to36(&rp_cBuffer[idx]);
			rhDevice->WriteData36(rhUnit, data36);
			if (st = rhDevice->CheckWordCount(rhUnit))
				return st;
		}
	}
...
int rp_ReadIO(UNIT *rpUnit, int32 reg, int32 *data)
{
	RPUNIT *rpData   = (RPUNIT *)rpUnit->uData;
...
int rp_WriteIO(UNIT *rpUnit, int32 reg, int32 data)
{
	RPUNIT *rpData = (RPUNIT *)rpUnit->uData;
	int    st = EMU_OK;
	char   *diskName = rpUnit->dType->Name;
...
#ifdef DEBUG
	if (dbg_Check(DBG_IOREGS) && (st != EMU_ILR))
		dbg_Printf("%s: %s (%02o) <= %06o\n",
			diskName, rp_regNames[reg >> 1], reg, data);
#endif DEBUG
...
int rp_Process(UNIT *rpUnit)
{
	...
	switch (function) {
		...
		case FNC_SEARCH:
			...
			st = disk_Seek(rpUnit, diskAddr, SEEK_SET);
#ifdef DEBUG
