...
INSTRUCTION *pdp10_extOpcode[01000];
...
int cpu_pInterrupt; // Interrupt Pending
...
// Set Program Flags
inline void cpu_SetFlags(int36 newFlags)
{
	if (FLAGS & PC_USER) {
		newFlags |= PC_USER;
		if (!(FLAGS & PC_USERIO))
...
// Calculate for effective address
inline int36 p10_ksGetEffAddr(int36 data)
{
	int18 Indirect;
	int18 Index;
	int36 eTemp;
	...
	do {
		...
		if (Index)
			eAddr = VMA(eAddr + RH(p10_vRead(Index, eaMode)));
...
inline int36 p10_ksCalcJumpAddr(int36 jData, int eaMode)
{
	int18 Indirect;
	int18 Index;
	int36 jAddr;
	...
	do {
		...
		if (Index) {
			mem_vRead36(Index, &jTemp, eaMode);
#ifdef DEBUG
			if (dbg_Check(DBG_TRACE|DBG_OPERAND))
		...
		if (Indirect) {
			mem_vRead36(VMA(jAddr), &jData, eaMode);
#ifdef DEBUG
			if (dbg_Check(DBG_TRACE|DBG_OPERAND))
...
// Calculate for byte pointer address
inline int36 p10_ksCalcBPAddr(int36 bpData, int eaMode)
{
	int18 Indirect;
	int18 Index;
	int36 bpAddr;
	...
	do {
		Indirect = INST_GETI(bpData);
		Index    = INST_GETX(bpData);
		bpAddr   = INST_GETY(bpData);
		
		if (Index) {
			mem_vRead36(Index, &bpTemp, eaMode);
			bpAddr = (bpAddr + RH(bpTemp)) & VMA_MASK;
		}

		if (Indirect) {
			mem_vRead36(bpAddr, &bpData, eaMode);
//			if (cpu_pInterrupt)
//				emu_Abort(p10_SetJump, P10_INTERRUPT);
		}
...
inline int p10_CheckPXCT(int prvFlags)
{
	int newMode = 0;
...
inline void p10_Execute(int30 xAddr)
{
	...
	if (pager_On && (FLAGS & PC_TRAPS)) {
		...
		trapFlag = (FLAGS & PC_TRAPS) >> 25;
		trapAddr = ((FLAGS & PC_USER) ?
...
//	printf("CPU: UUO code %03o at PC %06llo\r\n", opCode, RH(PC));
	...
	if (EBR & PG_EBR_M_TOPS20_PAGING) {
		...
		p10_pWrite(uptAddr + MUUO_T20_OPCODE,
			LH(FLAGS & ~PC_TRAPS) | LHSR(HR & MUUO_M_OPCODE), 0);
	...
	FLAGS = LH(BR) | ((FLAGS & PC_USER) ? PC_PCU : 0);
	DO_JUMP(RH(BR));
}
...
#ifdef DEBUG
	if (dbg_Check(DBG_TRACE|DBG_DATA))
		dbg_Printf("ADJSP: %06llo,,%06llo + %llo,,%llo = %06llo,,%06llo\n",
			LHSR(AR), RH(AR), eAddr, eAddr, LHSR(BR), RH(BR));
#endif DEBUG
}
...
void p10_Opcode_IBP(void)
{
	...
	if (opAC) {
		...
		if (Size) {
			...
			AR = (AR & ~(BP_M_POS|BP_M_ADDR))
				| ((Pos << BP_P_POS) & BP_M_POS) | (PTR & BP_M_ADDR);
...
void p10_Opcode_ILDB(void)
{
	...
	BR = p10_ksCalcBPAddr(AR, eaMode);
	mem_vRead36(BR, &BRX, byteMode);
...
void p10_Opcode_MOVE(void)
{
	...
	mem_vRead36(eAddr, &curAC[opAC], dataMode);
...
void p10_Opcode_MOVEM(void)
{
	...
	mem_vWrite36(eAddr, curAC[opAC], dataMode);
...
void p10_Opcode_MOVES(void)
{
	...
	mem_vRead36(eAddr, &AR, dataMode);
	mem_vWrite36(eAddr, AR, dataMode);
	if (opAC)
		curAC[opAC] = AR;
}
...
//	mem_vWrite36(eAddr, SWAP36(curAC[opAC]), dataMode);
...
void p10_Opcode_BLT(void)
{
	// Move R[E]-R(AC)+1 words starting with (L(AC))->(R(AC))

	int30 srcAddr, dstAddr;
	int30 endAddr, lenAddr;
	int st;
...
#ifdef DEBUG
	if (endAddr < dstAddr) {
		dbg_Printf("BLT: *** Program Error at PC %06llo ***\n", RH(PC - 1));
...
#ifdef DEBUG
	if (dbg_Check(DBG_TRACE|DBG_DATA))
		dbg_Printf("BLT: SA: %06o -> DA: %06o at %06o Words\n",
			srcAddr, dstAddr, (lenAddr < 1 ? 1 : lenAddr));
#endif DEBUG

#if defined(KL10) || defined(KS10)
	...
	srcMode = p10_CheckPXCT(PXCT_BLT_SRC) | PTF_BLT;
	dstMode = p10_CheckPXCT(PXCT_BLT_DST) | PTF_BLT;

	// ACTION: Clear Core need to be implemented.

	// Normal Block Transfer
	do {
//		if (cpu_pInterrupt)
//			break;
		if (st = mem_vRead36(VMA(srcAddr), &BR, srcMode))
			break;
		if (st = mem_vWrite36(VMA(dstAddr), BR, dstMode))
			break;
		srcAddr++;
		dstAddr++;
	} while (dstAddr <= endAddr);

//	if (st || cpu_pInterrupt) {
	if (st) {

#ifdef DEBUG
		if (dbg_Check(DBG_TRACE|DBG_DATA))
			dbg_Printf("BLT: *** Interrupt at PC %06o\n", RH(PC-1));
#endif DEBUG

#if defined(KL10) || defined(KS10)
		// Must update SA/DA on AC for KL/KS Processors
		AR = XWD((int36)srcAddr, (int36)dstAddr);
		curAC[opAC] = SXT36(AR);
#endif

		if (st) p10_PageFailTrap2();
	}
...
void p10_Opcode_JRST(void)
{
	switch(opAC) {
		...
		case 02:
			...
			eaMode = p10_CheckPXCT(PXCT_EA);
			eaFlags = p10_ksCalcJumpAddr(HR, eaMode);
			if (FLAGS & PC_USER) {
				eaFlags |= PC_USER;
				if (!(FLAGS & PC_USERIO))
					eaFlags &= ~PC_USERIO;
		...
		case 07:
			...
			if (FLAGS & PC_USER)
				...
				mem_vWrite36(eAddr++, FLAGS, NOPXCT);
				mem_vWrite36(eAddr++, PC, NOPXCT);
				mem_vRead36(eAddr++, &AR, NOPXCT);
				mem_vRead36(eAddr, &BR, NOPXCT);
				AR &= PC_FLAGS;
				if (FLAGS & PC_USER) {
		...
		case 014:
			...
			if (FLAGS & PC_USER)
				...
				mem_vWrite36(eAddr, FLAGS & PC_FLAGS, NOPXCT);
			break;
...
void p10_Opcode_XCT(void)
{
	...
	if (opAC && ((FLAGS & PC_USER) == 0)) {
			cpu_pFlags |= opAC;
#ifdef DEBUG
/*
			dbg_SetMode(DBG_TRACE|DBG_DATA);
			dbg_Printf("PXCT: Attempt to switch to user mode\n");
*/
#endif DEBUG
	}
...
// For KI10, KL10, and KS10 only, otherwise no-op instruction
// 257 MAP - Map
void p10_Opcode_MAP(void)
{
	...
	if ((FLAGS & PC_USER) && ((FLAGS & PC_USERIO) == 0))
		...
		dataMode = p10_CheckPXCT(PXCT_DATA);
		AR = p10_GetMap(eAddr, dataMode);
		curAC[opAC] = SXT36(AR);
#ifdef DEBUG
		if (dbg_Check(DBG_DATA))
			dbg_Printf("MAP: %06llo,,%06llo -> %06llo,,%06llo\n",
				LHSR(eAddr), RH(eAddr), LHSR(AR), RH(AR));
#endif DEBUG
	}
}
...
void p10_Opcode_PUSHJ(void)
{
	...
	AR = AOB(AR);
	BR = FLAGS | RH(PC);
	mem_vWrite36(RH(AR), BR, stackMode);
	curAC[opAC] = SXT36(AR);

	FLAGS &= ~(PC_FPD|PC_AFI|PC_TRAPS);
	if (LH(AR) == 0)
		FLAGS |= PC_TRAP2;
...
#ifdef DEBUG
	if (dbg_Check(DBG_DATA))
		dbg_Printf("PUSHJ: Stack: %06llo,,%06llo <- %06llo,,%06llo\n",
			LHSR(AR), RH(AR), LHSR(BR), RH(BR));
#endif DEBUG
}
...
#ifdef DEBUG
	if (dbg_Check(DBG_DATA))
		dbg_Printf("PUSH: Stack: %06llo,,%06llo <- %06llo,,%06llo\n",
			LHSR(AR), RH(AR), LHSR(BR), RH(BR));
#endif DEBUG
}
...
void p10_Opcode_POP(void)
{
	...
	AR = curAC[opAC];
	mem_vRead36(RH(AR), &BR, stackMode);
	mem_vWrite36(eAddr, BR, dataMode);

#ifdef DEBUG
	if (dbg_Check(DBG_DATA))
		dbg_Printf("POP: Stack: %06llo,,%06llo -> %06llo,,%06llo\n",
			LHSR(AR), RH(AR), LHSR(BR), RH(BR));
#endif DEBUG
...
#ifdef DEBUG
	if (dbg_Check(DBG_DATA))
		dbg_Printf("POPJ: Stack: %06llo,,%06llo -> %06llo,,%06llo\n",
			LHSR(AR), RH(AR), LHSR(BR), RH(BR));
#endif DEBUG
...
void p10_Opcode_JSR(void)
{
	...
	mem_vWrite36(eAddr, (FLAGS & PC_FLAGS) | RH(PC), NOPXCT);
	FLAGS &= ~(PC_FPD|PC_AFI|PC_TRAP1|PC_TRAP2);
	DO_JUMP(eAddr + 1);
}
...
void p10_Opcode_JSA(void)
{
	...
	mem_vWrite36(eAddr, curAC[opAC], NOPXCT);
	AR = RHSL(eAddr) | RH(PC);
	curAC[opAC] = SXT36(AR);
	DO_JUMP(eAddr + 1);
}
...
void p10_Opcode_JRA(void)
{
	...
	AR = curAC[opAC];
	mem_vRead36(LHSR(AR), &ARX, NOPXCT);
...
void p10_Opcode_SUBI(void)
{
	...
	p10_spSubtract(&curAC[opAC], eAddr);
}
...
void p10_Opcode_CAI(void)
{
...
#ifdef DEBUG
	if (dbg_Check(DBG_TRACE|DBG_DATA)) {
		dbg_Printf("CAI: %c%012llo %c%012llo\n",
			((AR < 0) ? '-' : '+'), AR & WORD36_ONES,
			((eAddr < 0) ? '-' : '+'), eAddr & WORD36_ONES);
	}
#endif DEBUG
	...
	switch(opCode & 7) {
		case 0:                           break;
		case 1: if (AR < eAddr)  DO_SKIP; break;
		case 2: if (AR == eAddr) DO_SKIP; break;
		case 3: if (AR <= eAddr) DO_SKIP; break;
		case 4:                  DO_SKIP; break;
		case 5: if (AR >= eAddr) DO_SKIP; break;
		case 6: if (AR != eAddr) DO_SKIP; break;
		case 7: if (AR > eAddr)  DO_SKIP; break;
	}
}
...
void p10_Opcode_JUMP(void)
{
	...
	switch(opCode & 7) {
		case 0:                              break;
		case 1: if (AR <  0) DO_JUMP(eAddr); break;
		case 2: if (AR == 0) DO_JUMP(eAddr); break;
		case 3: if (AR <= 0) DO_JUMP(eAddr); break;
		case 4:              DO_JUMP(eAddr); break;
		case 5: if (AR >= 0) DO_JUMP(eAddr); break;
		case 6: if (AR != 0) DO_JUMP(eAddr); break;
		case 7: if (AR >  0) DO_JUMP(eAddr); break;
	}
}
...
static int idleInstruction (int36 insn, int18 AC, int30 *nextPC)
{
	...
	*nextPC = (*nextPC + 1) & VMA_MASK;
	...
	switch (INST_GETOP (insn)) {
		...
		case 0332: // SKIPE
		case 0336: // SKIPN
			E = p10_ksGetEffAddr (insn);
			dataMode = p10_CheckPXCT(PXCT_DATA);
			mem_vRead36(E, &AR, dataMode);
			AR = WORD36_SXT(AR);
...
// 415 XMOVEI Extended Move Immediate
...
void p10_Opcode_SETOM(void)
{
	...
	mem_vWrite36(eAddr, WORD36_XONES, dataMode);
}
...
void p10_Opcode_HLL(void)
{
	...
	mem_vRead36(eAddr, &BR, dataMode);
	curAC[opAC] = LHSXT(BR) | RH(curAC[opAC]);
}
...
#if defined(KL10) || defined(KS10)
	curAC[opAC] = LHSXT(eAddr) | RH(curAC[opAC]);
#else
	curAC[opAC] = RH(curAC[opAC]);
#endif
}
...
void p10_Opcode_HLRZ(void)
{
	...
	mem_vRead36(eAddr, &BR, dataMode);
#ifdef BUG704
	if ((PC == 0353630) && (opAC == 003) && (eAddr == 011)) {
		// A bug in TOPS-10 cause very first block corrupted
		// from tape drive.  A bug need to being fixed in
		// TOPS-10 v7.04 source code (AUTCON.MAC).  Once a bug is
		// fixed, this patch may be removed.

		// Keep only MASSBUS drive number and get rid unncessary CS2.BAI
		// and rest of any bits.
		BR &= 0000007000000LL;
	}
#endif BUG704
	curAC[opAC] = LHSR(BR);
...
#ifdef DEBUG
	if (dbg_Check(DBG_DATA)) {
		dbg_Printf("HLRZ: %06llo,,%06llo -> %06llo,,%06llo\n",
			LHSR(BR), RH(BR), LHSR(curAC[opAC]), RH(curAC[opAC]));
	}
#endif DEBUG
}
...
int p10_Go(void)
{
	...
	// Create a new process for this KS10 Processor itself.
	// This did not work with current sockets, etc!
//	pid = fork();
//	if (pid < 0) {
//		perror("KS10: fork");
//		return EMU_OK;
//	} else if (pid > 0) {
//		printf("KS10 Processor started on child process %d.\n", pid);
//		p10_pid = pid;
//		return EMU_OK;
//	}

	// Initialize sockets on this process.
//	p10_ctyInitialize();
	...
	timer_Start();
	timer_SetAlarm(p10_HandleTimer);
...
//		p10_ctyCleanup();
	...
	while (emu_State == EMU_RUN) {
		pager_PC   = PC; // Save base address for trap.
		cpu_pFlags = 0;  // Reset process flags.
...
void p10_Initialize(void)
{
	...
	basOpcode[0] = p10_ksOpcode_UUO;

	// Initialize LUUO opcodes
	for (i = 0001; i <= 0037; i++)
		basOpcode[i] = p10_ksOpcode_LUUO;

	// Initialize MUUO opcodes
	for (i = 0040; i <= 0177; i++)
		basOpcode[i] = p10_ksOpcode_UUO;
	...
	basOpcode[0700] = p10_ksOpcode_IO700;
	basOpcode[0701] = p10_ksOpcode_IO701;
	basOpcode[0702] = p10_ksOpcode_IO702;

	basOpcode[0704] = p10_Opcode_UMOVE;
	basOpcode[0705] = p10_Opcode_UMOVEM;

	basOpcode[0710] = p10_ksOpcode_TIOE;
	basOpcode[0711] = p10_ksOpcode_TION;
	basOpcode[0712] = p10_ksOpcode_RDIO;
	basOpcode[0713] = p10_ksOpcode_WRIO;
	basOpcode[0714] = p10_ksOpcode_BSIO;
	basOpcode[0715] = p10_ksOpcode_BCIO;
	basOpcode[0716] = p10_ksOpcode_BLTBU;
	basOpcode[0717] = p10_ksOpcode_BLTUB;

	basOpcode[0720] = p10_ksOpcode_TIOEB;
	basOpcode[0721] = p10_ksOpcode_TIONB;
	basOpcode[0722] = p10_ksOpcode_RDIOB;
	basOpcode[0723] = p10_ksOpcode_WRIOB;
	basOpcode[0724] = p10_ksOpcode_BSIOB;
	basOpcode[0725] = p10_ksOpcode_BCIOB;
	...
	for (i = 1; pdp10_Instruction[i].Name; i++) {
		...
		if (Flags & OP_KS) {
			if (!(Flags & (OP_EXT|OP_IO|OP_DEV|OP_FUNC))) {
				if (pdp10_Opcode[Opcode]->Opcode == 0)
